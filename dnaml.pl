...
use constant BINARY => '/usr/local2/sheldon/bin/dnaml';

# Command line arguments for DNAML (not exhaustive)
...
# 5 BOOLEAN reconstruct hypothetical sequences
# i non PHYLIP option for input file (required)
# c BOOLEAN non-phylip option to do consensus tree after boot strap (default yes)

# dnaml-spcific option string
# Letters followed by a colon expect an argument
# Letters without a colon are BOOLEAN
my $option_string = 'G5IcSM:T:F:J:O:i:';
...
sub check_options {
    my %opt = @_;
    # currently no unique options for DNAML
}
