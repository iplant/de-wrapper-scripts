...
if (! `grep p_id $cannedReference` || ! `grep CDS $cannedReference`) {
    system "/usr/local2/sheldon/util/cuffdiff_fix_annotations.pl $cufflinksp $cannedReference $fasta";
}
...
# sort the data some
system "/usr/local2/sheldon/cuffdiff_get_goodstuff_test.pl $fdr";
